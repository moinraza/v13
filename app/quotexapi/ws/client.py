...
class WebsocketClient(object):
    ...
    def on_message(self, wss, message):
        ...
        try:
            ...
            if "51-" in str(message):
                ...
                message_settings = json.loads(message[0].get("settings", {}))
                message[0]["settings"] = message_settings
                ...
                self.api.realtime_price_data = message[0]
    ...
    def on_open(self, wss):
        ...
        self.wss.send(f'42["authorization",{json.dumps(auth_data)}]')
        self.wss.send('42["tick"]')
        self.wss.send('42["indicator/list"]')
        self.wss.send('42["drawing/load"]')
        self.wss.send('42["pending/list"]')
        self.wss.send(f'42["instruments/update",{json.dumps(instruments_update)}]')
